...
{upgrade_div.innerHTML+="<font color=green>"+(bioreactor_level+1)*50+"  матана, </font>"; upgrade_butt.disabled=false;}
else
{upgrade_div.innerHTML+="<font color=red>"+(bioreactor_level+1)*50+"  матана, </font>"; upgrade_butt.disabled=true;}
if (ressource['metan']>= (bioreactor_level+1)*50)
upgrade_div.innerHTML+="<font color=green>"+(bioreactor_level+1)*50+"  метана.</font>";
else
{upgrade_div.innerHTML+="<font color=red>"+(bioreactor_level+1)*50+"  метана.</font>";upgrade_butt.disabled=true;}
...
{upgrade_div_r.innerHTML+="<font color=green>"+Math.ceil((robot_level+1)*100+Math.pow(1.1, robot_level)-1)+"  метана. </font>"; upgrade_butt_r.disabled=false;}
else
{upgrade_div_r.innerHTML+="<font color=red>"+Math.ceil((robot_level+1)*100+Math.pow(1.1, robot_level)-1)+"  метана. </font>"; upgrade_butt_r.disabled=true;}
...
{upgrade_div_u.innerHTML+="<font color=green>"+Math.ceil(unhuman_level*50)+"  метана,</font>"; upgrade_butt_u.disabled=false;}
else
{upgrade_div_u.innerHTML+="<font color=red>"+Math.ceil(unhuman_level*50)+"  метана,</font>";upgrade_butt_u.disabled=true;}
if (ressource['religion']<=0)
{upgrade_div_u.innerHTML+="<font color=green> отрицательный или нулевой уровень религиозности</font>";}
else
{upgrade_div_u.innerHTML+="<font color=red> отрицательный или нулевой уровень религиозности</font>"; upgrade_butt_u.disabled=true;}
...
{ros_div.innerHTML+="<font color=green>"+(rosatiy_l*25)+"  смехуечек.</font>"; ros_but.disabled=false;}
else
{ros_div.innerHTML+="<font color=red>"+(rosatiy_l*25)+"  смехуечек.</font>";ros_but.disabled=true;}
upgrade_div_u.innerHTML+="</b>";
}
...
{upgrade_div_s.innerHTML+="<font color=green>"+science_level*100+"  метана. </font>"; upgrade_butt_s.disabled=false;
}
else
{upgrade_div_s.innerHTML+="<font color=red>"+science_level*100+"  метана. </font>"; upgrade_butt_s.disabled=true;}
...
if (tmp>0)
{news.value+="Гражданами Империи было произведено "+ tmp + " говна";
ressource['govno']+=tmp;}
...
if (tmp>0)
{news.value+="\n"+ Math.ceil(gov_to_met*(bioreactor_level+1))+ " говна было использовано в производстве.";
news.value+="При этом выработано "+ Math.ceil(gov_to_met*(bioreactor_level+1)/g_t_m_koeff)+ " метана";
...
news.value+="При этом выработано"+ tmp+ " метана";
ressource['metan']=safe_plus(ressource['metan'], tmp);
ressource['govno']=0; }
}
...
if (tmp>0)
{news.value+="\n"+ tmp+ " граждан умерло из-за недостатка метана.";
...
news.value+="\nПоголовье клерикалов уменьшилось на "+ (tmp-ressource['priest'])+ " единиц.";
else if (tmp<ressource['priest'])
news.value+="\nПоголовье клерикалов возросло на "+ (ressource['priest']-tmp)+ " единиц.";
...
news.value+="\nКоличество появившихся гуманитариев: "+ tmp+ ".";}
...
news.value+="\nСилами ученых и нелюдей было уничтожено "+ (res_unhuman['work0']+res_scientist['work2'])+ " религиозного дурмана.";
}
...
{
news.value+="\nУчеными было произведено "+ Math.ceil(res_scientist['work1']*matan_koeff)+ "  матана.";
news.value+="\nПри этом израсходовано "+ Math.ceil(res_scientist['work1']*matan_metan_koeff)+ " метана.";
...
news.value+="\nУчеными было произведено "+ Math.ceil(ressource['metan']/matan_metan_koeff*matan_koeff)+ "  матана.";
news.value+="\nПри этом израсходован весь доступный метан.";
...
{
news.value+="\nНелюди произвели "+ Math.ceil(res_unhuman['work2']*smeh_koeff)+ "  смехуечек.";
ressource['smeh']= safe_plus(ressource['smeh'], res_unhuman['work2']*smeh_koeff);
}
else {
news.value+="\nНелюди произвели "+ Math.ceil(ressource['matan'])+ "  смехуечек.";
...
{
news.value+="\nРоботами было произведено "+ Math.ceil(res_robot['work2']*bydlo_metan_koeff)+ "  метана.";
news.value+="\nПри этом было убито "+ res_robot['work2']+ " граждан Империи.";
...
news.value+="\nРоботами было произведено "+ Math.ceil(ressource['bydlo']*bydlo_metan_koeff)+ "  метана.";
news.value+="\nПри этом были убиты все граждане Империи\n".
...
news.value+="\nОбучено "+ (ressource['scientist']-tmp)+ "  ученых.";

if (ressource['scientist']>ressource['metan'])
{
tmp=ressource['scientist'];
news.value+="\n"+ (ressource['scientist']-ressource['metan'])+ "  ученых погибло из-за недостатка метана.";
...
{
news.value+="\n"+ ressource['unhuman']-Math.ceil(ressource['metan']/metan_unhuman_koeff)+ "  нелюдей погибло из-за недостатка метана.";
...
{
news.value+="\nРоботами было произведено "+ res_robot['work0']+ "  роботов.";
news.value+="\nПри этом было израсходовано "+ Math.ceil(res_robot['work0']*metan_robot_koeff)+ " метана.";
...
{
news.value+="\nРоботами было произведено "+ Math.ceil(ressource['metan']/metan_robot_koeff)+ "  роботов.";
news.value+="\nПри этом был израсходован весь метан.";
...
news.value+="\nКлерикалы произвели "+ (ressource['priest'])+ " религиозного дурмана.";}
...
{
news.value+="\nВ результате бунтов уничтожено "+ (ressource['metan']-safe_plus(ressource['metan'],-rel*ressource['bydlo']/(ressource['matan']+1)))+ "  метана, "+ (ressource['matan']-safe_plus(ressource['matan'],-rel*ressource['bydlo']/(ressource['matan']+1)))+ "  матана.";
...
news.value+="\nГуманитариями уничтожено "+ (ressource['matan']-t1)+ "  матана, "+ (ressource['smeh']-t2)+ "  смехуечек.";
news.value+="\nГуманитариями съедено "+ (ressource['govno']-t3)+ "  говна.";
...
{
news.value+="\nПоздравяем! Развитие науки и сопряженное с ним вымирание быдла привело к эволюционному скачку!\n Вчерашняя научная элита теперь считается быдлом, часть нелюдей стала учеными, часть слилась с роботами.\n Все клерикалы и гуманитарии старого образца выброшены на свалку истории.";
...
ros_div.innerHTML="<font color=red>По улицам потекла кровь, а страна получила много метана.</font>"
}
